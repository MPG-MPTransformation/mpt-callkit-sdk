...
public class BitmapUtils {
    ...
    public static Bitmap getBitmap(ImageProxy image) {
        ...
        ByteBuffer nv21Buffer = yuv420ThreePlanesToNV21(image.getImage().getPlanes(), image.getWidth(),
                image.getHeight());
     ...
     * values. For an
     * image of size S, the first S positions of the array contain all the Y values.
     * The remaining
     * positions contain interleaved V and U values. U and V are subsampled by a
     * factor of 2 in both
     * dimensions, so there are S/4 U values and S/4 V values. In summary, the NV21
     * array will contain
     ...
     * are subsampled
     * by a factor of 2 in both dimensions. {@link Image#getPlanes} returns an array
     * with the Y, U and
     * V planes. The Y plane is guaranteed not to be interleaved, so we can just
     * copy its values into
     * the first part of the NV21 array. The U and V planes may already have the
     * representation in the
     * NV21 format. This happens if the planes share the same buffer, the V buffer
     * is one position
     * before the U buffer and the planes have a pixelStride of 2. If this is case,
     * we can just copy
