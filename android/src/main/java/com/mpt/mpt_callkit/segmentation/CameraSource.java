 ...
 * 
 * <p>Supports automatic camera resolution selection based on device capabilities:
 * <ul>
 *   <li>{@link #RESOLUTION_LOW} - 640x1080 for low-end devices</li>
 *   <li>{@link #RESOLUTION_MEDIUM} - 720x1280 for mid-range devices</li>
 *   <li>{@link #RESOLUTION_HIGH} - 1080x1920 for high-end devices</li>
 *   <li>{@link #RESOLUTION_AUTO} - Automatically selects based on device specs (default)</li>
 * </ul>
 * 
 * <p>Usage example:
 * <pre>
 * CameraSource cameraSource = new CameraSource(activity);
 * cameraSource.setResolutionMode(CameraSource.RESOLUTION_AUTO); // Default
 * // or
 * cameraSource.setCustomResolution(640, 1080); // Custom resolution
 * </pre>
 */
public class CameraSource {
  ...
  // Frame processing optimization - process every Nth frame to increase FPS
  // Set to 1 to process all frames, 2 to process every 2nd frame, 3 for every 3rd frame, etc.
  // This matches iOS frameSkipInterval logic
  ...
  // Async processing executor (matching iOS videoProcessingQueue)
  ...
  public synchronized CameraSource start() throws IOException {
    ...
    synchronized (processorLock) {
      if (frameProcessor != null) {
        Log.d(TAG, "Starting frame processor");
        frameProcessor.start();
      }
    }
    
    System.out.println("SDK-Android: CameraSource start");
    return this;
  }
  ...
  public synchronized void stop() {
    if (processingThread != null) {
      try {
        System.out.println("SDK-Android: CameraSource stop");
  ...
  private Camera createCamera() throws IOException {
    ...
    // Update resolution based on current mode before selecting size
    updateRequestedResolution();
    
    SizePair sizePair = sizePair =
          selectSizePair(
              camera,
              requestedWidth,
              requestedHeight);
  ...
  private class FrameProcessingRunnable implements Runnable {
    ...
    // Frame skipping for performance optimization (matching iOS logic)
    ...
    public void run() {
      ...
      while (true) {
        ...
        // Skip processing if previous frame is still being processed (matching iOS logic)
        if (isProcessingFrame) {
          // Return buffer to camera and skip this frame
          camera.addCallbackBuffer(data.array());
          continue;
        }
        
        // Apply frame skip interval for FPS control (matching iOS logic)
        ...
        // Set processing flag and use async processing (matching iOS videoProcessingQueue)
        isProcessingFrame = true;
        final ByteBuffer finalData = data;
        final Camera finalCamera = camera;
        
        videoProcessingExecutor.execute(new Runnable() {
          @Override
          public void run() {
            try {
              synchronized (processorLock) {
              ...
              // Reset processing flag and return buffer to camera
              isProcessingFrame = false;
              finalCamera.addCallbackBuffer(finalData.array());
            }
          }
        });
      }
