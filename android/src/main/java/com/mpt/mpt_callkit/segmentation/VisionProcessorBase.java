...
import com.mpt.mpt_callkit.segmentation.BitmapUtils;
import com.mpt.mpt_callkit.segmentation.CameraImageGraphic;
import com.mpt.mpt_callkit.segmentation.FrameMetadata;
import com.mpt.mpt_callkit.segmentation.InferenceInfoGraphic;
import com.mpt.mpt_callkit.segmentation.ScopedExecutor;
...
public abstract class VisionProcessorBase<T> implements VisionImageProcessor {
  ...
  // Whether this processor is started and ready to process frames
  ...
  /**
   * Starts the vision processor. This should be called when the camera starts.
   * Initializes timers and resources needed for processing.
   */
  @Override
  public synchronized void start() {
    ...
    Log.d(TAG, "Starting VisionProcessorBase");
    isStarted = true;
  }
  
  /**
   * Restarts the vision processor after it has been stopped.
   * This recreates necessary resources for processing.
   */
  public synchronized void restart() {
    ...
    if (isShutdown) {
      // Reset shutdown state for restart
      isShutdown = false;
      resetLatencyStats();
    }
  ...
  /**
   * Checks if the vision processor is currently started and ready to process frames.
  ...
  public synchronized void processByteBuffer(
    ...
    // Don't process if not started or shutdown
    if (!isStarted || isShutdown) {
      return;
    }
  ...
  private Task<T> setUpListener(
    ...
    return task.addOnSuccessListener(
            ...
            results -> {
              ...
              VisionProcessorBase.this.onSuccess(results, originalCameraImage, frameStartMs);
  ...
  public void stop() {
    synchronized (this) {
      ...
      Log.d(TAG, "Stopping VisionProcessorBase");
