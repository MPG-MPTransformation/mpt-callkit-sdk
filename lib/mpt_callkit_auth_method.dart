import 'dart:convert';
import 'dart:convert' as convert;

import 'package:http/http.dart' as http;
import 'package:mpt_callkit/logger/mpt_callkit_logger.dart';
import 'package:mpt_callkit/mpt_aes_helper.dart';

class MptCallkitAuthMethod {
  final _logger = MptCallkitLogger.instance;
  final _headers = {
    "Content-Type": "application/json",
    "Accept": "*/*",
  };

  final loginAPI = "/api/TokenAuth/Authenticate";
  final loginSSOAPI = "/api/TokenAuth/ExternalSystemAuthenticate";
  final logoutAPI = "/acd-asm-chat/agent-status/logout";

  Future<bool> login({
    required String username,
    required String password,
    required int tenantId,
    String? baseUrl,
    Function(String?)? onError,
    Function(Map<String, dynamic>)? data,
  }) async {
    final body = {
      "tenantId": tenantId,
      "userNameOrEmailAddress": username,
      "password": password,
    };

    bool status = false;

    _logger.logMessage("[Mpt_API_Auth] Login body: ${jsonEncode(body)}");
    _logger.logMessage(
        "[Mpt_API_Auth] Login API: ${baseUrl ?? "https://crm-dev-v2.metechvn.com"}$loginAPI");
    try {
      final response = await http.post(
        Uri.parse("${baseUrl ?? "https://crm-dev-v2.metechvn.com"}$loginAPI"),
        headers: _headers,
        body: jsonEncode(body),
      );

      if (response.statusCode == 200 || response.statusCode == 201) {
        final responseData = jsonDecode(response.body);

        data?.call(responseData);
        _logger.logMessage('[Mpt_API_Auth] Login Response data: $responseData');
        if (responseData != null) {
          if (responseData["success"]) {
            var result = responseData["result"];
            if (result != null) {
              status = result["status"];
              _logger.logMessage("[Mpt_API_Auth] Login status: $status");
              if (!status) {
                onError?.call(
                    "[Mpt_API_Auth] Username or password is incorrect. Message: [${result["message"]}]");
              } else {
                _logger.logMessage("[Mpt_API_Auth] Login successfully");
              }
              return status;
            }
            onError?.call("[Mpt_API_Auth] Login failed, result is null");
            return false;
          }
          onError
              ?.call("[Mpt_API_Auth] Login failed, cannot get data response");
          return false;
        }
        onError?.call("[Mpt_API_Auth] Login failed, cannot get data response");
        return false;
      } else {
        _logger.logMessage(
            '[Mpt_API_Auth] Failed to post data. Status code: ${response.statusCode}');
        onError?.call(
            "[Mpt_API_Auth] Login failed, please check your internet connection");
        return false;
      }
    } catch (e) {
      _logger.logMessage("[Mpt_API_Auth] Error in login: $e");
      onError?.call("[Mpt_API_Auth] Login failed with error: $e");
      return false;
    }
  }

  Future<bool> loginSSO({
    required String ssoToken,
    required String organization,
    String? baseUrl,
    Function(String?)? onError,
    Function(Map<String, dynamic>)? data,
  }) async {
    bool status = false;

    final body = {
      "partner_session": ssoToken,
      "partner_tenant": organization,
    };

    _logger.logMessage("Login SSO body: ${jsonEncode(body)}");
    _logger.logMessage(
        "Login SSO API: ${baseUrl ?? "https://crm-dev-v2.metechvn.com"}$loginSSOAPI");

    try {
      final response = await http.post(
        Uri.parse(
            "${baseUrl ?? "https://crm-dev-v2.metechvn.com"}$loginSSOAPI"),
        headers: _headers,
        body: jsonEncode(body),
      );

      if (response.statusCode == 200 || response.statusCode == 201) {
        final responseData = jsonDecode(response.body);
        data?.call(responseData);
        _logger.logMessage('Login SSOResponse data: $responseData');
        if (responseData != null) {
          if (responseData["success"]) {
            var result = responseData["result"];
            if (result != null) {
              status = result["status"];
              _logger.logMessage("[Mpt_API_Auth] Login SSO status: $status");
              if (!status) {
                onError?.call(
                    "[Mpt_API_Auth] Incorrect credentials. Message: [${result["message"]}]");
              } else {
                _logger.logMessage("[Mpt_API_Auth] Login successfully");
              }
              return status;
            }
            onError?.call("[Mpt_API_Auth] Login failed, result is null");
            return false;
          }
          onError
              ?.call("[Mpt_API_Auth] Login failed, cannot get data response");
          return false;
        }
        onError?.call("[Mpt_API_Auth] Login failed, cannot get data response");
        return false;
      } else {
        _logger.logMessage(
            '[Mpt_API_Auth] Failed to post data. Status code: ${response.statusCode}');
        onError?.call(
            "[Mpt_API_Auth] Login failed, please check your internet connection");
        return false;
      }
    } catch (e) {
      _logger.logMessage("[Mpt_API_Auth] Error in login SSO: $e");
      onError?.call("[Mpt_API_Auth] Login SSO failed with error: $e");
      return false;
    }
  }

  Future<bool> logout({
    String? baseUrl,
    Function(String?)? onError,
    required String cloudAgentName,
    required int cloudAgentId,
    required int cloudTenantId,
  }) async {
    try {
      final body = {
        "cloudAgentId": cloudAgentId,
        "cloudTenantId": cloudTenantId,
        "cloudAgentName": cloudAgentName
      };

      final response = await http.post(
          Uri.parse(
              "${baseUrl ?? "https://crm-dev-v2.metechvn.com"}$logoutAPI"),
          headers: _headers,
          body: jsonEncode(body));

      if (response.statusCode == 200 || response.statusCode == 201) {
        final responseData = jsonDecode(response.body);
        _logger.logMessage('Logout Response data: $responseData');
        if (responseData != null) {
          if (responseData["success"] && responseData["status"]) {
            return true;
          }
          return false;
        }
        return false;
      } else {
        _logger.logMessage(
            '[Mpt_API_Auth] Failed to post data. Status code: ${response.statusCode}');
        onError?.call(
            "[Mpt_API_Auth] Logout failed, please check your internet connection");
        return false;
      }
    } catch (e) {
      _logger.logMessage("[Mpt_API_Auth] Error in logout: $e");
      onError?.call("[Mpt_API_Auth] Logout failed with error: $e");
      return false;
    }
  }

  String decryptText(String encrypted) => MptAESHelper.decryptAesB64(encrypted);

  // Get current user info
  Future<Map<String, dynamic>?> getCurrentUserInfo({
    required String baseUrl,
    required String accessToken,
    Function(String?)? onError,
  }) async {
    final headers = {
      "Content-Type": "application/json",
      "Accept": "*/*",
      "Authorization": "Bearer $accessToken",
    };
    const userInfoApi = "/api/services/app/Session/GetCurrentLoginInformation";
    try {
      final response = await http.get(
        Uri.parse("$baseUrl$userInfoApi"),
        headers: headers,
      );

      if (response.statusCode == 200 || response.statusCode == 201) {
        final responseData = convert.jsonDecode(response.body);
        if (responseData != null) {
          if (responseData["success"]) {
            var result =
                convert.jsonDecode(decryptText(responseData["result"]));
            _logger.logMessage("CurrentUserInfo: $result");
            return result;
          }
          return null;
        }
        return null;
      } else {
        _logger.logMessage(
            '[Mpt_API] Failed to get data. Status code: ${response.statusCode}');
        onError?.call(
            '[Mpt_API] Failed to get data. Status code: ${response.statusCode}');
        return null;
      }
    } catch (e) {
      _logger.logMessage("[Mpt_API] Error in get current user info: $e");
      onError?.call("[Mpt_API] Get current user info with error: $e");
      return null;
    }
  }

  // Get all conversation
  Future<dynamic> getConfiguration({
    required String baseUrl,
    required String accessToken,
    Function(String?)? onError,
  }) async {
    final headers = {
      "Content-Type": "application/json",
      "Accept": "*/*",
      "Authorization": "Bearer $accessToken",
    };

    final url =
        "$baseUrl/config/EncryptedAbpUserConfiguration/GetConfiguration";

    try {
      final response = await http.get(Uri.parse(url), headers: headers);

      if (response.statusCode == 200 || response.statusCode == 201) {
        final responseData = convert.jsonDecode(response.body);
        if (responseData["success"]) {
          var result = convert.jsonDecode(decryptText(responseData["result"]));
          _logger.logMessage("[Mpt_API] GetAll: ${result.isNotEmpty}");
          return result;
        }
        onError?.call("[Mpt_API] Failed to get data");
        return null;
      }
      onError?.call(
          "[Mpt_API] Failed to get data with status code: ${response.statusCode}");
      return null;
    } catch (e) {
      _logger.logMessage("[Mpt_API] Error in getConfiguration: $e");
      onError?.call("[Mpt_API] Get all conversation with error: $e");
      return null;
    }
  }
}
